/* Programa que define un struct para almacenar distintos datos de alumnos de un 
 * centro incluyendo su nombre, e-mail y notas. Crea con ello un array de
 * alumnos y ofrece al usuario un menú de opciones para gestionar dicho array.
 * añadiendo 2 opciones nuevas, ordenar la lista de alumnos alfabéticamente y
 * ordenar el listado por nota de examen de forma descendente.
 * 
 * Alvaro Castillo
 */
...
class Practica_4_2
{
	...
	enum opciones
	{ SALIR, NUEVO, BORRAR, VER, APROBADOS, BUSCAR, ALFABETICAMENTE, NOTA }	
	...
	static void Main()
	{
		...
		do
		{
			...
			Console.WriteLine((int)opciones.ALFABETICAMENTE + 
				". Mostrar alumnos en orden alfabético");
			Console.WriteLine((int)opciones.NOTA + 
				". Mostrar alumnos por nota descendente");
				...
				case opciones.ALFABETICAMENTE:
					if(cantidad>0)
					{
						for (int i = 0; i < cantidad; i++)
						{
							...
							{
								if(alumnos[i].nombre.
								CompareTo(alumnos[j].nombre)>0)
								{
									nuevoAlumno = alumnos[i];
									alumnos[i] = alumnos[j];
				...
				case opciones.NOTA:
					if(cantidad>0)
					{
						for (int i = 0; i < cantidad; i++)
						{
							...
							{
								if(alumnos[j].
								notas.teoria < alumnos[j+1].notas.teoria)
								{
									nuevoAlumno = alumnos[j];
									alumnos[j] = alumnos[j+1];
									alumnos[j+1] = nuevoAlumno;
								}
							}
						}
						for(int i = 0; i < cantidad; i++)
